...
_SCHOLARPUBRE = r'class="gsc_a_ac gs_ibl">([\w-]*)'
    ...
    def _get_authorlist(self, authorinfo):
        ...
        for i in text.split(','):
            ...
            if bool(re.search(r'\d', i)):
                continue
            if ("Proceedings" in i or "Conference" in i or "Journal" in i or "Book" in i or
...
##                        _SCHOLARPUBRE, val.a['href'])[0]
    ...
    def citedby(self) -> _SearchScholarIterator or list:
        ...
        if not self.filled:
            self.fill()
        return _SearchScholarIterator(self.nav, self.citation_link)
